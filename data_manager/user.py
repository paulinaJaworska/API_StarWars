...
def is_data_validate(user_name, first_password, verified_password):
    if is_empty(user_name) \
            and is_empty(first_password) \
            and is_empty(verified_password):
        return True
    else:
        return False
...
def get_id_by_username(username):
    if is_already_in_db(username):
        user_id = db_user.get_id_by_username(username)
        return user_id['id']
    else:
        return 0
